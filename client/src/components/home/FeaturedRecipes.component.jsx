...
import {
    ...
    selectAllFeaturedRecipes,
    selectFeaturedStatus,
} from "../../store/slices/featured_recipes.slice";
    ...
    return (
        <article className="rounded-2xl overflow-hidden border border-[#2b1e2b] bg-gradient-to-br from-[#0b0710] to-[#221322] shadow-lg transform-gpu transition hover:-translate-y-1 hover:shadow-2xl" aria-label={ item.title }>
            <div onMouseEnter={ () => setPaused( true ) } onMouseLeave={ () => setPaused( false ) } className="relative w-full h-44 sm:h-48 bg-slate-900">
                ...
                { slides.length > 1 && (
                    <>
                        <button onClick={ goPrev } aria-label="Previous image" className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/50 text-white rounded-full w-8 h-8 flex items-center justify-center">‹</button>
                        <button onClick={ goNext } aria-label="Next image" className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/50 text-white rounded-full w-8 h-8 flex items-center justify-center">›</button>
...
export default function FeaturedRecipes( { pageSize = 6 } ) {
    const dispatch = useDispatch();
    const itemsFromStore = useSelector( selectAllFeaturedRecipes ) || [];
    const status = useSelector( selectFeaturedStatus );
    useEffect( () => {
        dispatch( fetchFeaturedRecipes( { page: 1, pageSize } ) );
    }, [ dispatch, pageSize ] );

    const demoData = [
        { id: "t-demo-1", title: "Blueberry Pancakes", category: "Breakfast", cuisine: "American", description: "Fluffy pancakes with blueberry.", ingredients: [ { name: "Flour" }, { name: "Blueberries" } ], images: [ makePlaceholder( "Blueberry Pancakes" ) ] },
        { id: "t-demo-2", title: "Spicy Pumpkin Risotto", category: "Main", cuisine: "Italian", description: "Creamy pumpkin risotto.", ingredients: [ { name: "Rice" } ], images: [ makePlaceholder( "Spicy Pumpkin Risotto" ) ] },
        { id: "t-demo-3", title: "Black Garlic Ramen", category: "Main", cuisine: "Japanese", description: "Rich umami broth ramen.", ingredients: [ { name: "Noodles" } ], images: [ makePlaceholder( "Black Garlic Ramen" ) ] },
        { id: "t-demo-4", title: "Witch's Herb Salad", category: "Salad", cuisine: "Fusion", description: "Fresh mixed herbs and leaves.", ingredients: [ { name: "Lettuce" } ], images: [ makePlaceholder( "Witch's Herb Salad" ) ] },
        { id: "t-demo-5", title: "Charred Veg Skewers", category: "Grill", cuisine: "Mediterranean", description: "Smoky vegetable skewers.", ingredients: [ { name: "Bell pepper" } ], images: [ makePlaceholder( "Charred Veg Skewers" ) ] },
        { id: "t-demo-6", title: "Dark Chocolate Mousse", category: "Dessert", cuisine: "French", description: "Silky dark chocolate mousse.", ingredients: [ { name: "Chocolate" } ], images: [ makePlaceholder( "Dark Chocolate Mousse" ) ] },
    ];
    ...
    const hasRealData = Array.isArray( itemsFromStore ) && itemsFromStore.length > 0;
    const listToRender = hasRealData ? itemsFromStore.slice( 0, pageSize ).map( normalizeItem ).filter( Boolean ) : demoData.slice( 0, pageSize );
