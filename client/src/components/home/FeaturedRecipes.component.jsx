...
function DemoCard( { item } ) {
    // Normalize images -> array of URL strings (works with images[0].url)
    const rawImages =
...
export default function FeaturedRecipes( { pageSize = 6 } ) {
    ...
    const itemsFromStore =
        useSelector( selectAllFeaturedRecipes ) || [];
        ...
        return {
            ...
            ingredients: Array.isArray( r.ingredients )
                ...
                : Array.isArray( r.components )
                    ? r.components
                    : [],
            // keep raw r.images; DemoCard will normalize (objects with .url or strings)
    ...
    const hasRealData =
        Array.isArray( itemsFromStore ) &&
        itemsFromStore.length > 0;
    const listToRender = hasRealData
        ? itemsFromStore
            .slice( 0, pageSize )
            .map( normalizeItem )
            .filter( Boolean )
